...
export default class ProjectConfigs {
    ...
    static async update(formData, isPasswordUpdate = false) {
        ...
        if (isPasswordUpdate) {
            ...
            if (newPassAgain !== newPass) {
                return {
                    level: 'error',
                    message:
                        'New Password & Confirmation Password do not match!',
                };
            }
            ...
            if (success) {
                ...
                return {
                    level: 'error',
                    message:
                        'Error updating password, check error logs for more info.',
                };
        }
        ...
        const newsletterShowLatestPosts =
            formData['newsletter.show_latest_posts'];
        const newsletterShowSubscription =
            formData['newsletter.show_subscription'];
        const newsletterShowFeaturedImage =
            formData['newsletter.show_featured_image'];
        const newsletterFooterContent = formData['newsletter.footer_content'];
        const newsletterCustomSubject =
            formData['newsletter.custom_subject_pattern'];
        const newsletterPoweredByGhost =
            formData['newsletter.show_powered_by_ghost'];
        const newsletterPoweredByGhosler =
            formData['newsletter.show_powered_by_ghosler'];
        ...
        if (!Array.isArray(email) || email.length === 0) {
            return {
                level: 'error',
                message: 'Add at-least one email configuration.',
            };
        }
        ...
        if (
            ...
            return {
                level: 'error',
                message: 'Ghost URL, Admin API Key or Secret is missing.',
            };
        }
        ...
        if (ghostAdminSecret.toString().length < 8) {
            return {
                level: 'error',
                message: 'Secret should at-least be 8 characters long.',
            };
        }
        ...
        configs.newsletter.show_comments =
            newsletterShowComments === 'on' ?? true;
        configs.newsletter.show_latest_posts =
            newsletterShowLatestPosts === 'on' ?? false;
        configs.newsletter.show_subscription =
            newsletterShowSubscription === 'on' ?? false;
        configs.newsletter.show_featured_image =
            newsletterShowFeaturedImage === 'on' ?? true;
        configs.newsletter.show_powered_by_ghost =
            newsletterPoweredByGhost === 'on' ?? true;
        configs.newsletter.show_powered_by_ghosler =
            newsletterPoweredByGhosler === 'on' ?? true;
        ...
        configs.custom_template.enabled =
            customTemplateEnabled === 'on' ?? false;
        ...
        if (success) {
            ...
            return {
                level: 'error',
                message:
                    'Error updating settings, check error logs for more info.',
            };
    }
    ...
    static async #getConfigFilePath() {
        ...
        const debugConfigPath = path.resolve(
            __dirname,
            '../../configuration/config.local.json',
        );
        const prodConfigPath = path.resolve(
            __dirname,
